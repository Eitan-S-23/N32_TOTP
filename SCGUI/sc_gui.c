...
int _SC_DrawRound(SC_tile *dest,SC_arc *p,int start_deg, int end_deg,uint16_t ac,uint16_t bc,SC_arc_dot *arc_dot)
{
    ...
    int16_t sx,sy,ex,ey;
    uint16_t *dest_out,tfc,tbc;
    int16_t x, y,dx,dy,dy2,temp;
    if(start_deg!=0||end_deg!=360)
    {
        ...
        {
            big = (end_deg+360 - start_deg > 180)?1:0;
        }
            ...
            if(big!=0xff)
            {
                ...
                if (!in_range)                  //弧外
                {
                    ...
                    if(arc_dot==NULL)                    //无内存
                    {
                        ...
                        dx =  SC_MIN(sd, ed);
                        tfc=(dx<255) ? alphaBlend(ac,tbc,SC_MIN(255-dx, alpha)):tbc;
                    }
                    else
                    {
                        dx= SC_get_dot(arc_dot,x,y);    //圆头读点
                        tfc=(dx<255)?alphaBlend(ac,tbc,dx):ac;
                    }
                }
            }
            *dest_out=alphaBlend(tfc,*dest_out,alpha*gui->alpha/255);
        }
    }
    if((dest==&pfb)&&SC_pfb_Refresh(&pfb,0))
    {
        ...
        for ( x = intersection.xs; x <=intersection.xe; x++,dest_out++)
        {
            ...
            if (temp<r1)
            {
                ...
                if(bc!=gui->bkc&&x<=gui->lcd_area.xe&& y>=gui->lcd_area.ys)
                {
                    ...
                    {
                        alpha = (temp - rmin) *inv_inner>>8; //内边缘抗锯齿
                        tbc=alphaBlend(ac,bc,alpha);
                    }
                    *dest_out=alphaBlend(tbc,*dest_out,gui->alpha);
                }
                else
                {
                    if (temp>rmin)
