...
#define  FIL               SDL_RWops

u8 f_read(FIL *fs,u8 *buff,UINT size,UINT *br)
{
    u8 ret;
    ret=  SDL_RWread(fs,buff, size, 1);
    return !ret;
}
UINT f_tell(FIL *fs)
{
    return SDL_RWtell(fs);
}
int f_lseek(FIL *fs,UINT offset)
{
    return SDL_RWseek(fs,offset, RW_SEEK_SET);
}
...
{
    SC_pfb_DrawFill(NULL,x0,y0,x0+len-1,y0,color);
}

void piclib_fill(u16 x,u16 y,u16 width,u16 height,u16 color)
{
    SC_pfb_DrawFill(NULL,x,y,x+width-1,y+height-1,color);
}
//填充颜色
void piclib_fill_color(u16 x,u16 y,u16 width,u16 height,u16 *color)
{
    SC_Refresh(x,y,x+width-1,y+height-1,color);
}
...
void piclib_init(void)
{
    ...
    pic_phy.draw_point=gui->bsp_pset;	//画点函数实现
...
u8 gif_getinfo(FIL *file,gif89a * gif)
{
    ...
    if(gif->gifLSD.flag&0x80)//存在全局颜色表
    {
        ...
        if(gif_readcolortbl(file,gif,gif->numcolors)) res|= 1;//读错误
    }
    printf("\r\n gif_getinfo err=%d",res);
    return res;
}
...
int gif_getnextbyte(FIL *gfile,gif89a* gif)
{
    ...
    while((Code=gif_getnextcode(gfile,gif))>=0)
    {
        if(Code==gif->lzw->ClearCode)
        {
            ...
            if(gif->lzw->ClearCode>=(MAX_NUM_LWZ_BITS))return -1;//Error
            //Clear the tables
    ...
    do
    {
        ...
        case GIF_INTRO_IMAGE://图像描述
            ...
            if(gif->gifISD.flag&0x80)//存在局部颜色表
            {
                ...
                if(gif_readcolortbl(gfile,gif,numcolors))return 1;//读错误 colortbl
            }
...
//===============================DEMO=============================================
static gif89a tgif89a;			//gif89a文件 静态内存 1.1K
static LZW_INFO tlzw;			//lzw        静态内存 15.6K
static FIL *gfile;
static gif89a *mygif89a=&tgif89a;

u8 gifdecoding=0;//标记GIF正在解码.

void gif_demo_cb(void *arg)
{
    Event *e=arg;
    mygif89a->lzw=&tlzw;
    u16 width=320;
...
//           }
    ...
    if(res==2)
    {
        x=0;
        y=0;
        printf("\r\n gif89a_size=%d LZW_INFO_size=%d",sizeof(gif89a),sizeof(LZW_INFO));
        gfile = SDL_RWFromFile("SCGUI/gif/test.gif", "rb");
        if(gfile)//打开文件ok
        {
            ...
            if(mygif89a->gifLSD.width>width||mygif89a->gifLSD.height>height)
            {
                ...
                printf("\r\n PIC_SIZE_ERR- w=%d,h=%d",mygif89a->gifLSD.width,mygif89a->gifLSD.height);
            }
            else
            {
