...
///DMA中断完成标志
#define dma_wait_ok()  //while(JL_SPI1->CON& (1<<13))
#include "n32wb03x.h"
#include "LCD.h"
...
#if 1
    dma_wait_ok();
    LCD_Color_Fill( xs,  ys,  xe,  ye,color);
#else
    //-----------------无DMA------------------------
    for(int y = ys; y <=ye; y++)
    {
...
int SC_pfb_clip(SC_tile *clip, int xs,int ys,int xe,int ye,uint16_t colour)
{
    ys= SC_MAX(ys,0);
    xs= SC_MAX(xs,0);
    xe= SC_MIN(xe,LCD_SCREEN_WIDTH-1);
    ye= SC_MIN(ye,LCD_SCREEN_HEIGHT-1);
...
///计算相交区
int SC_pfb_intersection(SC_tile *dest,SC_AREA *p,int xs,int ys,int xe,int ye)
{
...
//抛物线
void SC_Draw_parabola(int x0, int y0, int x1, int y1)
{
    // 确定遍历方向和范围
    int xs = x0 - (x1+1) / 2 ;
    int xe = x0 + (x1+1) / 2 ;
    ...
    for (int y = ys; y <= ye; y ++)
    {
        for (int x = xs; x <= xe; x++)
        {
            int dx2 = (x - x0) * (x - x0);
            int k_dx2 = k * dx2;
            int parabola_y = (k_dx2 >> 12) + y0 + y1;
            if ((y1 > 0 && y < parabola_y) || (y1 < 0 && y > parabola_y))
            {
                //gui->bsp_pset(x, y, 255);
            }
            else
            {
                gui->bsp_pset(x, y, 255);
