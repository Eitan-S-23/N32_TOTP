...
static inline uint16_t get_bpp_value(const uint8_t *buffer,uint16_t offset, const uint8_t bpp)
{
    ...
    case 8:
        ...
        break;
    case 4:
        alpha= (buffer[offset / 2] >> (4 * (1 - (offset & 1)))) & 0x0F;
        alpha = (alpha << 4) + alpha;  // 替代 alpha *= 17
        break;
    case 2:
        alpha= (buffer[offset / 4] >> (2 * (3 - (offset & 3)))) & 0x03;
        alpha= alpha*85;
        break;
    case 1:
        alpha= (buffer[offset / 8] >> (7 - (offset & 7))) & 0x01 ? 255 : 0;
        break;
    default:
        break;
    }
...
///显示一个lvgl字符
int SC_pfb_lv_letter(SC_tile *dest,SC_AREA *align,int xs,int ys,lv_font_glyph_dsc_t *dsc,uint32_t unicode, lv_font_t *font,uint16_t fc,uint16_t bc)
{
    SC_tile pfb;
    SC_AREA intersection;
    //先计算再对齐
    int offs_y= ys+(font->line_height-dsc->box_h-dsc->ofs_y-font->base_line);
    ...
    for ( y = intersection.ys; y <=intersection.ye; y++, dest_offs+= dest->w)
    {
        ...
        for ( x = intersection.xs; x <=intersection.xe; x++)
        {
            ...
            if (src_x >= 0 && src_x< dsc->box_w&&src_y >= 0 && src_y < dsc->box_h)
            {
                ...
                *dest_out=alphaBlend(fc,*dest_out,alpha);
            }
        }
    }
...
///长文本显示自动换行，支持\n换行
void SC_pfb_printf(SC_tile *dest,SC_AREA *box, int x,int y,const char* txt,uint16_t fc,uint16_t bc, lv_font_t* font)
{
...
///设置坐标对齐到parent
void SC_set_align(SC_AREA *parent,int *xs,int *ys,int w,int h,uint8_t align)
{
    if(parent!=NULL)
    {
...
///显示文本
void SC_pfb_str(SC_tile *dest,int x,int y,const char *text,uint16_t fc,uint16_t bc,lv_font_t* font,SC_AREA *parent,uint8_t align)
{
    ...
    lv_font_glyph_dsc_t g[16];
    uint32_t unicode[16];
    //--------实际长度------
    /* j<16 防止长文本越界写栈：原循环只按像素宽度退出，16pt 以下字体
     * + 160 px 宽能塞 >16 个字形，会踩坏相邻栈变量引发 HardFault。 */
    for(len=0; len<width && j<16; len+=g[j++].adv_w)
    {
...
///显示标签
void SC_pfb_label(SC_tile *dest,int x,int y,SC_AREA *parent,SC_label *label,uint16_t lab_bc)
{
    if(label->font)
    {
...
///文本框
void SC_pfb_text_box(SC_tile *dest,SC_AREA *parent,int xs,int ys,SC_label* label)
{
    SC_AREA text_box;
    int w=5;
    int h=32;
